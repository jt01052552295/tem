...
for($i=0; $i<count($siteMenu); $i++):
	...
	$siteMenu[$i]['url'] = str_replace("http://localhost:8080/crayon/ko", ".", $siteMenu[$i]['url']);
	// echo $siteMenu[$i]['url']."<Br>";
	
	// 폴더 만들기
	/*
	if(strlen($siteMenu[$i]['mCode'])==2 && $siteMenu[$i]['me_make'] != false):
	...
	// 파일만들기
	/*
	if(strlen($siteMenu[$i]['mCode'])==4 && $siteMenu[$i]['mCode'] != '50'):
		$file = $siteMenu[$i]['url'];
