;(function($){
	...
	$(window).scroll(function() {
		...
		var offset = $('.vg_inner').height() / 2;
		var range = $('.vg_inner').height();
		var scOpa =  1 - (scTop/offset);
		if(scOpa<0) scOpa = 0;

		if(scTop > 0){

			$(".vg_title").stop().animate({'margin-top':(scTop/2)+'px','opacity':scOpa},1000)

			// $("#header").addClass("m-fixed");
		    //$(".header_wrap").addClass("active");
        } else {
        	$(".vg_title").stop().animate({'margin-top':'0px','opacity':'1.0'},1000)
