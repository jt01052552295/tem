...
if(!preg_match("([^[:space:]]+)",$name)) {
	die("error,".$infodu['lang']['mail']['txt14']);
}

//▶ 이메일 내용이 입력되었는지 확인
if(!preg_match("([^[:space:]]+)",$mail)) {
	die("error,".$infodu['lang']['mail']['txt15']);
}
...
// 제목 내용설정하기
$subject=$name."님 ".$act." --".$admin_home;

//메일 내용합치기

////////////////메일 내용합치기////////////////[수정가능]항목이 추가되면 아래와 같은 방식으로 추가해주십시오.

$content=$admin_home." 에서 작성된 <b>".$name."</b>님으로 부터의 <b>".$act."</b> 입니다.<br />";
...
if(!mail($to, $subject, $mailbody, $header)) {
	die("error,".$infodu['lang']['mail']['txt18']);
} else { 
	die("ok,".$infodu['lang']['mail']['txt19']);
}
?>