...
$siteMenu[] = array(mCode => "1010", title => "회사개요", 	url => $routeName."/overview.php", pid => "overview", me_link => true, me_use => true );
$siteMenu[] = array(mCode => "1020", title => "인사말", 	url => $routeName."/message.php", pid => "message", me_link => true, me_use => true );
...
$siteMenu[] = array(mCode => "1050", title => "오시는길", 	url => $routeName."/location.php", pid => "location", me_link => true, me_use => true );
...
$siteMenu[] = array(mCode => "2010", title => "사업영역", url =>$routeName."/business.php", pid => "business", me_link => true, me_use => true );


$routeName = G5_LANG_URL."/performance";
$siteMenu[] = array(
	mCode 		=> "30",	
	title 		=> "실적현황",		
	url 		=> $routeName."/performance.php",		  
	makeDir		=> "performance",	
	pid 		=> "",	
	me_link		=> false,	
	me_use 		=> true
);
$siteMenu[] = array(mCode => "3010", title => "실적현황", url =>$routeName."/performance.php", pid => "performance", me_link => true, me_use => true );
...
if($sca){ // 게시판 카테고리 처리
	...
	for($i=0; $i<count($nav2nd); $i++): 
	  if($nav2nd[$i]['title'] == $sca) $_menu_current_name = $nav2nd[$i]['pid'];
	endfor;
