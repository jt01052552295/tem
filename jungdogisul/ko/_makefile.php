...
for($i=0; $i<count($siteMenu); $i++):
	...
	$siteMenu[$i]['url'] = str_replace("http://localhost:8080/jungdogisul/ko", ".", $siteMenu[$i]['url']);
	//echo $siteMenu[$i]['url']."<Br>";
	
	// 폴더 만들기
	/*
	if(strlen($siteMenu[$i]['mCode'])==2 && $siteMenu[$i]['mCode'] != '50'  && $siteMenu[$i]['mCode'] != '60'):
		
		echo $siteMenu[$i]['pid']."<Br>";
	...
	*/
	...
	// 파일만들기
	/*
	if(strlen($siteMenu[$i]['mCode'])==4 && substr($siteMenu[$i]['mCode'],0,2) != '50' && substr($siteMenu[$i]['mCode'],0,2) != '60'):
		$file = $siteMenu[$i]['url'];
		echo $file."<Br>";
