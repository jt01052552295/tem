;(function($){
		...
		maxHeight = Math.max.apply(null, heights);
		 ...
		 $('#gnbnav').hover(
		 	function(){
				...
				// var len = $(this).find('.category_list > ul > li').length
				//var len = $(this).find('.subNavi > li').length
				//if(len>0){
					// $(this).find('.sub_category').addClass('on');
				...
				$topNavSub.stop().hide().animate({height:0},_menuShowSpeed);
	...
	$(window).scroll(function() {
		...
		var subPageWrap_h = $('#subPageWrap').height();

		//console.log(winHeight, dom_h, subPageWrap_h)

		 if(scTop > 0 && dom_h > 1000){
