;(function($){
	$.fn.do_animate_scroll = function(){
		...
		this.each(function(){
            ...
            var position = { top:_e.offset().top, left:_e.offset().left}
			...
			$(window).innerHeight() > position.top  ? _e.addClass("sos-animate") : _e.removeClass("sos-animate");
			

	        var lastScrollTop = $(window).scrollTop();
	        $( window ).scroll(function(){
	        	var st = $(this).scrollTop();
	        	//var scTop = st + $( window ).innerHeight();

	        	var o = window.pageYOffset, i = window.innerHeight;
	        	var scTop = o+i;

	        	//console.info(o, i , o+i)
	        	
				//console.info('aos2', st, $( window ).innerHeight())
				//console.info(scTop, target)
	        	if( st > lastScrollTop){
	        		if( !_e.hasClass("sos-animate")) {
	        			( scTop > target ) ? _e.addClass("sos-animate") : '';
	        		}
	        	} else {
	        		scTop > target ? '' : _e.removeClass("sos-animate");
	        	}
	        	lastScrollTop = st;
	        })
