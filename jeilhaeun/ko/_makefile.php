...
for($i=0; $i<count($siteMenu); $i++):
	...
	$siteMenu[$i]['url'] = str_replace("http://localhost:8080/jeilhaeun/ko", ".", $siteMenu[$i]['url']);
	//echo $siteMenu[$i]['url']."<Br>";
	
	// 폴더 만들기
	/*
	if(strlen($siteMenu[$i]['mCode'])==2 && $siteMenu[$i]['mCode'] != '50' && $siteMenu[$i]['mCode'] != '40'):
	...
	*/
	...
	// 파일만들기
	/*
	if(strlen($siteMenu[$i]['mCode'])==4 && strpos($siteMenu[$i]['mCode'], '50') === false && strpos($siteMenu[$i]['mCode'], '40') === false):
		$file = $siteMenu[$i]['url'];
		//echo $file."<Br>";
