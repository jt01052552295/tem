...
if(!preg_match("([^[:space:]]+)",$name)) {
	die("error,".$infodu['lang']['main']['mail']['error01']);
}

//▶ 이메일 내용이 입력되었는지 확인
if(!preg_match("([^[:space:]]+)",$tel)) {
	die("error,".$infodu['lang']['main']['mail']['error02']);
}

//▶ 이메일 내용이 입력되었는지 확인
if(!preg_match("([^[:space:]]+)",$email)) {
	die("error,".$infodu['lang']['main']['mail']['error03']);
}


//▶ 내용이 입력되었는지 확인
if(!preg_match("([^[:space:]]+)",$con)) {
	die("error,".$infodu['lang']['main']['mail']['error04']);
}
...
if(!mail($to, $subject, $mailbody, $header)) {
	die("error,".$infodu['lang']['main']['mail']['txt07']);
} else { 
	die("error,".$infodu['lang']['main']['mail']['txt08']);
}
?>