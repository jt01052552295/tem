;(function($){
		 ...
		 $('#gnbnav').hover(
		 	function(){
				...
				$(this).find('ul.subNavi').removeClass('on');
		 		$topNavBg.stop().hide().animate({height:0},0);
		 		$topNavSub.stop().hide().animate({opacity:'0', height:0},0);
		 	}
		 );
	...
	$(window).scroll(function() {
		...
		if(scTop > 0){
            ...
            $(".header_wrap").removeClass("topPosition").addClass("active");
        } else {
        	$("header").removeClass("m-fixed");
            $(".header_wrap").removeClass("active").addClass("topPosition");
        }      
