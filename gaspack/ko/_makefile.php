...
for($i=0; $i<count($siteMenu); $i++):
	...
	$siteMenu[$i]['url'] = str_replace("http://localhost:8080/gaspack/ko", ".", $siteMenu[$i]['url']);
	...
	if(strlen($siteMenu[$i]['mCode'])==2):
		...
		echo $siteMenu[$i]['makeFolder']."<Br>";
		//@rmdir('./'.$siteMenu[$i]['makeFolder']);
		
		@mkdir('./'.$siteMenu[$i]['makeFolder'], G5_DIR_PERMISSION);

		$file ='./'.$siteMenu[$i]['makeFolder'].'/_common.php';
		$f = @fopen($file, 'a+');
		fwrite($f, "<?php\n");
	...
	*/
	...
	if(strlen($siteMenu[$i]['mCode'])==6 && substr($siteMenu[$i]['mCode'],0 ,2) != '50'):
		$file = $siteMenu[$i]['url'];
